<?php
//
// Posql sample script using PEAR::DB driver
//
//    {{{ internal settings
...
if (PEAR::isError($db)) {
  abort($db); // this script is using original method for debug.
  // use the following sample code if you want to use PEAR::DB API methods.
  //die($db->toString());
  // or
  //die($db->getUserInfo());
  // or
...
<head>
<meta charset="UTF-8">
<title>Posql sample script using PEAR::DB driver</title>
<style type="text/css">
...
<body>
<h1>Posql for PEAR::DB - <span class="sub">sample script</span></h1>
<hr>
<dl>
<?php
$options = array(
...
foreach ($options as $option => $desc) {
  printf('<dt>%s</dt><dd>%s</dd>', $desc, $db->getOption($option));
}
...
foreach ($items as $url => $title) {
  $res = $db->execute($stmt, array($url, $title));
  if (PEAR::isError($res)) {
    abort($res);
  }
  debug($res, sprintf(str_replace('?', '%s', $sql), $url, $title));
}
$db->freePrepared($stmt);

// get next/last sequence id
debug($db->lastInsertId(), '$db->lastInsertId()');
debug($db->nextId(), '$db->nextId()');
...
debug($db->affectedRows(), '$db->affectedRows()');
...
debug($db->setOption("engine", "php"), '$db->setOption("engine", "php")');
