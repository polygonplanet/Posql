...
class Posql_Math {
  ...
  function format($number, $float = null, $sprintf = null) {
    ...
    if ($length > 0) {
      ...
      if ($float !== null && (($float - 0) === 0) && strpos($result, $dot) !== false) {
  ...
  function convertToBase($number, $frombase, $tobase) {
    static $bases = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    $result = '';
    ...
    if ($frombase > 0 && $frombase < 63 && $tobase > 0 && $tobase < 63) {
      $numbers = array();
  ...
  function hexdec($number, $strict = false) {
    ...
    if ($length > 1 && (!$strict || $this->isHex($number))) {
      if ($length >= 8) {
        $dec = 0;
        $bit = 1;
        if ($this->hasbc) {
